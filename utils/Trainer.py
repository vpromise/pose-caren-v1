...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.last_epoch, self.params.max_epoch):
            ...
            logger.info('Joint average err is {:.4f} pixel'.format(average_err))
            ...
            vis.plot('average_err', average_err)
            vis.log("epoch:{epoch},lr:{lr},loss:{loss},average_err:{average_err}".format(epoch=epoch, loss=self.loss_meter.value()[0],
            lr=get_learning_rates(self.optimizer),average_err=average_err))
